        ...
        {played ? (
          <>
            <div style={{display:"flex",alignItems:"center",gap:3}}>
              {result.scoreA!=null&&<span style={{fontSize:isMobile?18:22,fontWeight:900,color:winHome?"#111":"#999"}}>{result.scoreA}</span>}
              <span style={{fontSize:10,fontWeight:800,color:"#ccc"}}>–</span>
              {result.scoreB!=null&&<span style={{fontSize:isMobile?18:22,fontWeight:900,color:winAway?"#111":"#999"}}>{result.scoreB}</span>}
            </div>
          ...
          <span style={{fontSize:isMobile?11:(prominent?13:11),fontWeight:800,color:isBye?"#bbb":"#999",padding:isMobile?"2px 7px":"3px 8px",border:isBye?"none":"1px solid #eee",borderRadius:2}}>{isBye?"BYE":"VS"}</span>
        )}
      </div>
    );
    const awayCell = (
...
// Formats the year(s)/season(s) a streak touched, from an ordered list of {year,seasonNum} pairs
// (seasonNum may be missing on older data, which falls back to the plain year as before). A year
// can hold more than one finalized season, so "2026" alone is ambiguous: this collapses to
// "2026 S1" for a single season, "2026 S1-S2" when the streak crosses seasons within one year,
// and "2025 S2 – 2026 S1" when it crosses years.
function seasonSpanLabel(pairs){
  const uniq=[];
  pairs.forEach(p=>{const last=uniq[uniq.length-1];if(!last||last.year!==p.year||last.seasonNum!==p.seasonNum)uniq.push(p);});
  if(!uniq.length)return "";
  const fmt=p=>p.seasonNum?`${p.year} S${p.seasonNum}`:String(p.year);
  if(uniq.length===1)return fmt(uniq[0]);
  const first=uniq[0], last=uniq[uniq.length-1];
  if(first.year===last.year&&first.seasonNum&&last.seasonNum)return `${first.year} S${first.seasonNum}-S${last.seasonNum}`;
...
function longestStatStreak(games,pred){
  ...
  if(!max)return null;
  const span=seasonSpanLabel(games.slice(bestStart,bestEnd+1).map(g=>({year:g.year,seasonNum:g.seasonNum})));
...
function ProfileTab({history,setupRows,currentEntries,season,year,permanentUsers,sel,setSel,pTab,setPTab,articles,setActiveArticle,playerStats,gameArchive}) {
  const isMobile = useIsMobile();
  const veryNarrow = useIsMobile(360);
      ...
      {profile&&user&&<div ref={profileRef}><Card style={{borderTop:`3px solid ${RED}`,overflow:"hidden"}}>
        ...
        <div style={{padding:isMobile?12:18}}>
          ...
          {pTab==="streaks"&&(()=>{
            ...
            const wsSpan=maxW>0?seasonSpanLabel(allLogs.slice(bestWS,bestWE+1).map(w=>({year:w.year,seasonNum:w.seasonNum}))):"";
            ...
            let bestSSnLen=0,bestSSnYear=null,bestSSnSeasonNum=null,bestSSnTeam="";
            const ssnSrc=[...profile.seasons.filter(s=>!s.isHistorical)];
            if(profile.cur)ssnSrc.push({...profile.cur,year,seasonNum:season});
            ssnSrc.forEach(s=>{let c=0,m=0;(s.weekLog||[]).forEach(w=>{if(w.result==="win"){c++;if(c>m)m=c;}else c=0;});if(m>bestSSnLen){bestSSnLen=m;bestSSnYear=s.year;bestSSnSeasonNum=s.seasonNum;bestSSnTeam=s.teamName;}});
            ...
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              <div><SL>Streak Records</SL>
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:8}}>
                  ...
                  <SB label="Best Season Streak" val={bestSSnLen>0?bestSSnLen+"W":"—"} color="#007a00" sub={bestSSnLen>0?`${bestSSnTeam} · ${seasonSpanLabel([{year:bestSSnYear,seasonNum:bestSSnSeasonNum}])}`:""}/>
                </div>
              </div>
...
// ── EnterResultsPanel ─────────────────────────────────────────────────────
function EnterResultsPanel({entries,weekResults,setWeekResults,week,setWeek,applyBulkResults,applyWeekResults,reverseWeekResults,pointsToRestore,restoreSeasonPoints,teamsToRestore,restoreSeasonTeams,findResultMismatches,repairResultMismatches,postSeasonInputs,setPSI,applyPostSeason,finalizeSeason,season,setSeason,year,setYear,teamNames,schedule,history,onImportHistory,setupRows,saveToDb,setup,setSetup,postWeekRecapToGroupMe,postGameOfWeekPreview}) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      {teamsToRestore&&<Card style={{overflow:"hidden",borderLeft:`4px solid #b8860b`}}>
        <div style={{padding:"12px 16px",display:"flex",gap:12,alignItems:"center",flexWrap:"wrap"}}>
          <div style={{flex:1,minWidth:240}}>
            <div style={{fontSize:12,fontWeight:900,color:"#b8860b",textTransform:"uppercase",letterSpacing:0.5}}>⚠ {teamsToRestore.changes.length} coach{teamsToRestore.changes.length===1?"":"es"} changed team when Season {teamsToRestore.seasonNum} was finalized</div>
            <div style={{fontSize:11,color:"#666",marginTop:4,lineHeight:1.5}}>
              {teamsToRestore.changes.slice(0,5).map(c=><div key={c.user}>{c.user}: <strong>{c.to}</strong> → {c.from}</div>)}
              {teamsToRestore.changes.length>5&&<div style={{color:"#999"}}>+{teamsToRestore.changes.length-5} more…</div>}
              <div style={{marginTop:4}}>A saved Season {teamsToRestore.seasonNum+1} roster does this. If it's out of date, put the teams back — if the move was intended, no action is needed and you can ignore this.</div>
            </div>
          </div>
          <button onClick={()=>{
            if(!window.confirm(`Put ${teamsToRestore.changes.length} coach${teamsToRestore.changes.length===1?"":"es"} back on the team they played in Season ${teamsToRestore.seasonNum}?\n\n${teamsToRestore.changes.map(c=>`  • ${c.user}: ${c.from} → ${c.to}`).join("\n")}`))return;
            restoreSeasonTeams();
          }} style={{background:"#b8860b",color:"#fff",border:"none",borderRadius:2,padding:"9px 16px",cursor:"pointer",fontFamily:ff,fontSize:12,fontWeight:800,textTransform:"uppercase",letterSpacing:0.5,flexShrink:0}}>Restore teams</button>
        </div>
      </Card>}
      ...
      {entryWeek>=14&&postSeasonInputs&&(()=>{
        ...
        // The ranking lists (conference standings, recruiting, dynasty top 5) are seeded from the
        // roster when the season starts, so any roster change since then leaves them stale: the
        // dropdowns offer every current team, but picking one the list doesn't contain made the swap
        // below look up index -1 and write an undefined row — which blanked the whole screen on the
        // next render. Reconcile against the current roster instead: keep the order already set,
        // drop teams that have left, append anyone new at the bottom.
        const rankList=(list)=>{
          const seen=new Set(), out=[];
          (list||[]).forEach(r=>{if(r?.teamName&&teamNames.includes(r.teamName)&&!seen.has(r.teamName)){seen.add(r.teamName);out.push(r);}});
          teamNames.forEach(t=>{if(!seen.has(t)){seen.add(t);out.push({teamName:t});}});
          return out.map((r,i)=>({...r,rank:i+1}));
        };
        const swapRank=(field,list,i,teamName)=>{
          const j=list.findIndex(x=>x.teamName===teamName);
          if(j<0||j===i)return; // the picked team isn't in this list — do nothing rather than corrupt it
          const arr=[...list];
          [arr[i],arr[j]]=[arr[j],arr[i]];
          setPSI(prev=>({...prev,[field]:arr.map((r,k)=>({...r,rank:k+1}))}));
        };
        const recruitingList=rankList(psi.recruiting);
        const confStandingsList=rankList(psi.confStandings);
        const dynastyTop5List=rankList(psi.dynastyTop5);
        const prestigeGains=psi.prestigeGains||[];
...
export default function App() {
  ...
  function finalizeSeason() {
    ...
    // A saved season roster is a snapshot of who played what when it was last saved. Opening the
    // roster editor for a future season seeds it from the teams as they stood at that moment, so
    // one that predates a mid-season team change will quietly put those coaches back on their old
    // teams. Applying it silently is the bug; naming the changes lets it be caught or confirmed.
    const nextRoster = setup?.seasonRosters?.[newSeason];
    const rosterChanges=entries.map(e=>{
      const o=nextRoster?.find(r=>r.userId===e.userId);
      return o&&o.teamName&&o.teamName!==e.teamName?{user:e.userName,from:e.teamName,to:o.teamName}:null;
    }).filter(Boolean);
    const rosterNote=rosterChanges.length
      ? `\n\n⚠ The saved Season ${newSeason} roster moves ${rosterChanges.length} coach${rosterChanges.length===1?"":"es"} to a different team:\n${rosterChanges.map(c=>`  • ${c.user}: ${c.from} → ${c.to}`).join("\n")}\nIf that roster is out of date, cancel and clear it in League Setup first.`
      : "";
  ...
  // Teams a finalize moved off the team they actually played. The finalized standings hold each
  // coach's real team for that season, so a revert caused by a stale season roster is undoable.
  const teamsToRestore=(()=>{
    const h=lastFinalizedThisYear;
    if(!h||!Array.isArray(h.finalStandings))return null;
    const changed=entries.map(e=>{
      const prev=h.finalStandings.find(t=>e.userId&&t.userId&&t.userId===e.userId);
      return prev&&prev.teamName&&prev.teamName!==e.teamName?{user:e.userName,from:e.teamName,to:prev.teamName}:null;
    }).filter(Boolean);
    return changed.length?{seasonNum:h.seasonNum,changes:changed}:null;
  })();
  function restoreSeasonTeams(){
    const h=lastFinalizedThisYear;
    if(!h||!Array.isArray(h.finalStandings))return;
    const nextEntries=entries.map(e=>{
      const prev=h.finalStandings.find(t=>e.userId&&t.userId&&t.userId===e.userId);
      return prev&&prev.teamName?{...e,teamName:prev.teamName,userName:prev.userName||e.userName}:e;
    });
    // setup.rows is what the rest of the app reads for a coach's current team, so move it too or
    // the standings and the league setup disagree.
    const rows=(setup?.rows||[]).map(r=>{
      const prev=h.finalStandings.find(t=>r.userId&&t.userId&&t.userId===r.userId);
      return prev&&prev.teamName?{...r,teamName:prev.teamName}:r;
    });
    const updatedSetup={...setup,rows};
    setEntries(nextEntries);
    setSetup(updatedSetup);
    setTimeout(()=>saveToDb({entries:nextEntries,setup:updatedSetup}),100);
  }
      ...
      <div style={{maxWidth:1440,margin:"0 auto",padding:isMobile?"12px 16px":"24px",display:"grid",gridTemplateColumns:showSidebars?"260px 1fr 320px":"minmax(0,1fr)",gap:isMobile?12:24,alignItems:"start",boxSizing:"border-box"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:isMobile?10:14}}>
          ...
          {tab==="Home"?(
            ...
            <Card style={{overflow:"hidden",borderLeft:`5px solid ${RED}`}}>
              <div style={{padding:isMobile?"14px 14px 12px":"22px 26px 18px"}}>
                <div style={{fontSize:isMobile?20:32,fontWeight:900,color:"#111",textTransform:"uppercase",letterSpacing:-0.5,lineHeight:1.05}}>Dynasty Profiles</div>
                <div style={{fontSize:isMobile?12:14,color:"#555",marginTop:4,fontWeight:600}}>Meet the coaches and personalities building their dynasties.</div>
                <div style={{fontSize:10,color:"#999",marginTop:isMobile?8:10,fontWeight:700,textTransform:"uppercase",letterSpacing:0.5}}>{leagueName} • Season {season} • {year} • {week>13?"Post":`Week ${week}`}</div>
              </div>
            </Card>
          ):tab==="YearStats"?(
            <Card style={{overflow:"hidden",borderLeft:`5px solid ${RED}`}}>
              <div style={{padding:isMobile?"14px 14px 12px":"22px 26px 18px"}}>
                <div style={{fontSize:isMobile?20:32,fontWeight:900,color:"#111",textTransform:"uppercase",letterSpacing:-0.5,lineHeight:1.05}}>Year Stats</div>
                <div style={{fontSize:isMobile?12:14,color:"#555",marginTop:4,fontWeight:600}}>{leagueName} • Season {season} • {year} • {week>13?"Post":`Week ${week}`}</div>
              </div>
            </Card>
          ):(
            <Card style={{padding:isMobile?"10px 12px":"14px 16px",borderLeft:`4px solid ${RED}`}}>
              <div style={{fontSize:isMobile?15:18,fontWeight:900,color:"#111",textTransform:"uppercase"}}>{tab==="Redzone"?"Dynasty RedZone":tab==="Discord"?"Join Discord & Voice Chat":"League Rules"}</div>
