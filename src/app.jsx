            ...
            <label style={{background:weekParsing?"#ccc":RED,color:"#fff",fontSize:11,fontWeight:700,padding:"6px 12px",borderRadius:2,cursor:weekParsing?"wait":"pointer",fontFamily:ff,textTransform:"uppercase",letterSpacing:0.5,whiteSpace:"nowrap",opacity:weekParsing?0.7:1}}>
              ...
              <input ref={weekFileRef} type="file" accept="image/*" style={{display:"none"}} disabled={weekParsing} onChange={async e=>{
                ...
                try{
                  ...
                  const prompt=`You are parsing a College Football 27 dynasty schedule screenshot. Extract every visible week and its matchups. The image shows real school names for EVERY opponent regardless of whether that opponent is one of the dynasty teams or a CPU-controlled team — a real name being visible does NOT by itself mean a game is a CPU game. For each matchup, decide dynasty-vs-CPU using ONLY this rule: check the opponent's name against the "Valid dynasty team names" list below. (1) If it matches, or closely matches allowing for abbreviations like "St" for "State" or "Hawai'i" for "Hawaii", an entry in that list, this is a dynasty-vs-dynasty game — write the opponent using its EXACT name as it appears in the teams list, and do NOT prefix it with "CPU:" under any circumstances. (2) Only if the opponent's name does NOT match any entry in the teams list is it a non-dynasty CPU-controlled opponent — write it as its real school name prefixed with "CPU:" (e.g. "CPU:Ohio"), or the bare literal "CPU" only if no name is visible at all. Apply this same rule consistently to every matchup in the image — do not treat some pairs as dynasty games and others as CPU games inconsistently. If it is a bye week write "BYE". Respond with ONLY the JSON object below and absolutely nothing else — no explanation, no reasoning, no markdown fences, not even a single leading or trailing sentence: {"1":{"TeamA":"TeamB","TeamB":"TeamA"},"2":{"TeamA":"CPU:Florida State"},"3":{"TeamA":"BYE"}} Use only week numbers as keys. Only include weeks and teams visible in the image.${teamsText}`;
