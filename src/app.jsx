...
function ContentHub({sorted,entries,week,season,leagueName,history,leader,articles,setArticles,setActiveArticle}) {
  ...
  async function generate(type) {
    ...
    try {
      ...
      alert("Error generating content. Check your API connection.");
    }
    setGenerating(null);
  }
...
export default function App() {
  ...
  useEffect(() => {
    dbLoad().then(row => {
      if (row) {
        ...
        if (row.entries && row.entries.length) setEntries(row.entries);
  ...
  const saveToDb = useCallback(async (overrides = {}) => {
    try {
      await dbSave({
        setup: overrides.setup !== undefined ? overrides.setup : setup,
        season: overrides.season !== undefined ? overrides.season : season,
        week: overrides.week !== undefined ? overrides.week : week,
  ...
  }, [setup, season, week, entries, history, postSeasonInputs, articles]);
  ...
  async function processImage() {
    ...
    try {
      ...
      const response=await fetch(API_URL,{method:"POST",headers:HEADERS,body:JSON.stringify({model:MODEL,max_tokens:1000,messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:imageFile.type||"image/jpeg",data:base64}},{type:"text",text:`CF27 Scores screen. Dynasty teams: ${teamList}. For each visible team determine win/loss from score. Return ONLY raw JSON array:\n[{"teamName":"Troy","result":"win","ranked25":false,"ranked10":false}]\nOnly include dynasty teams you can see.`}]}]})});
      ...
      <div style={{maxWidth:1180,margin:"0 auto",padding:"12px 10px",display:"grid",gridTemplateColumns:isMobile?"1fr":"200px 1fr 260px",gap:12,alignItems:"start"}}>
          ...
          {tab==="Standings"&&<Card style={{overflow:"hidden"}}>
            ...
            <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              ...
              <tbody>{sorted.map((t,i)=>{const tot=calcTotal(t);const beh=leader-tot;return(<tr key={t.teamName} style={{borderBottom:"1px solid #eee",background:i===0?"#fff8f8":i%2===0?"#fafafa":"#fff"}}>
                ...
                <td style={{padding:"10px 7px",fontWeight:i===0?800:600,color:"#111",whiteSpace:"nowrap",borderRight:"1px solid #eee"}}>{t.teamName}</td>
        ...
        <div style={{display:isMobile?"none":"flex",flexDirection:"column",gap:12}}>
          <Card><CardHead>Top Headlines</CardHead><div style={{padding:"4px 0"}}>
            ...
            {articles.length===0&&<>
              {sorted.slice(0,3).map((t,i)=><div key={t.teamName} style={{padding:"9px 12px",borderBottom:"1px solid #f0f0f0"}}><div style={{fontSize:12,fontWeight:700,color:"#111",lineHeight:1.4}}>{i===0?"🏆":i===1?"📈":"📊"} {t.teamName} leads with {calcTotal(t)} pts</div><div style={{fontSize:10,color:"#888",marginTop:2}}>{t.wins}W–{t.losses}L</div></div>)}
